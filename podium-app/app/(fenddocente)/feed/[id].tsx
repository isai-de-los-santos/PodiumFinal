...
function RubricCriterion({ index, criterion, max, onChangeScore, disabled = false }: RubricCriterionProps) {
    ...
    return (
        <View style={[styles.criterionCard, disabled && { opacity: 0.65 }]}>
            <TouchableOpacity
...
export default function EvaluateProjectScreen() {
        ...
        const ensureProject = async () => {
            try {
                ...
                if (mounted) {
                    ...
                    // Verificar si este docente ya tiene una evaluación guardada
                    if (current && user) {
                        const myEval = current.evaluaciones_docentes?.find(
                            (e) => e.maestro_id === user.id
                        );
                        if (myEval) {
                            setIsLocked(true);
                            setHasSubmitted(true);
                            // Pre-cargar puntajes y retroalimentación previos
                            if (myEval.rubrica?.length > 0) {
                                setCriterios(myEval.rubrica);
                            }
                            setFeedback(myEval.retroalimentacion_final || "");
                        }
                    }
                }
            } catch (err) {
    ...
    const handleSubmit = useCallback(async () => {
        ...
        setSubmitting(false);
        if (res.ok) {
            // Bloquear edición tras enviar exitosamente
            setIsLocked(true);
            setHasSubmitted(true);
        } else {
            console.error("submitEvaluation error:", res.error);
        }
    }, [project, user, criterios, totalMax, totalScore, feedback, submitEvaluation]);

    // Botón inferior con 3 estados:
    // 1. Sin evaluación previa   → "Enviar mi Evaluación" → envía y bloquea
    // 2. Bloqueado               → "Editar Puntaje"       → desbloquea
    // 3. Desbloqueado (ya envió) → "Guardar Cambios"      → envía y bloquea
    ...
    return (
        <SafeAreaView style={styles.safe} edges={["top", "left", "right"]}>
            {/* Top bar — solo flecha atrás y título (se eliminó el botón Guardar) */}
            ...
            >
                ...
                <View style={styles.sectionCard}>
                    ...
                    <View style={styles.feedbackSection}>
                        ...
                        <TextInput
                            style={[styles.feedbackInput, isLocked && { opacity: 0.65, backgroundColor: colors.muted }]}
            ...
            {/* Bottom button — 3 estados */}
            <View style={[styles.bottomBar, { paddingBottom: insets.bottom + 16 }]}>
                <TouchableOpacity
                    style={[styles.submitButton, isLocked && styles.submitButtonLocked]}
                    onPress={handleBottomButton}
                    disabled={submitting}
                    activeOpacity={0.92}
                    accessibilityRole="button"
                    accessibilityLabel={isLocked ? "Editar puntaje" : hasSubmitted ? "Guardar cambios" : "Enviar evaluación"}
                >
